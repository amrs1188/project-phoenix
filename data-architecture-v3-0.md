# Project Phoenix: Data Architecture Document (v3.0)
* **Date:** October 19, 2025
* **Status:** Final - Aligned with Blueprint v8.0
...
The pipeline tracks the lead's journey. Standardized fast timers apply to all active assignments.
...
| `priority_level` | `INT` | Global dispatch priority (1-3) for Recycle leads. NULL if Fresh or Active. |
...
| `current_stage` | `VARCHAR(100)` | GHL pipeline stage name at last update. |
| `ghl_contact_id` | `VARCHAR(100) NOT NULL` | The GHL Contact ID within the assigned sub-account. |
| `timer_expires_at` | `TIMESTAMPTZ` | Deadline calculated based on assignment time and stage/lead type rules. |
...
| `cf_project_state_id` | `VARCHAR(100)` | The unique GHL Custom Field ID for "Project State" in this location. |
...
| `pipeline_id` | `VARCHAR(100)` | GHL Pipeline ID for the main sales pipeline in this location. |
...
### 2.6 (Optional) Table: `system_settings`
...
* **Master Source:** The PostgreSQL database is the absolute source of truth for lead status, history, and configuration.
* **GHL De-duplication:** GHL's native "prevent duplicate contact" feature (based on phone/email) must be enabled in all sub-accounts.
* **Handling Blanks:** Blank/NULL values from GHL custom fields are acceptable and will be stored as NULL in PostgreSQL.
